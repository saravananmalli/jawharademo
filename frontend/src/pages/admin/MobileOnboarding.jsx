...
import { Plus, Pencil, Trash2, Image, GripVertical, Smartphone, X } from 'lucide-react';
import {
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-extrabold text-gray-900 dark:text-white">Onboarding Screens</h1>
          </div>
      ...
      {loading && slides.length === 0 ? (
        ...
        <div className="text-center py-24">
          <Smartphone size={52} className="mx-auto mb-3 text-gray-200 dark:text-gray-700" />
          <h2 className="text-lg font-semibold text-gray-500 dark:text-gray-400 mb-1">No onboarding slides yet</h2>
        ...
        <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 transition-opacity duration-150 ${loading ? 'opacity-50' : 'opacity-100'}`}>
          {slides.map((s, index) => (
            <div
              key={s._id}
              draggable
            ...
            >
              {/* Portrait image */}
              <div className="h-56 bg-gray-100 dark:bg-gray-800 relative overflow-hidden shrink-0 flex items-center justify-center">
                {s.imageUrl ? (
                  <img
              ...
              <div className="p-3 flex-1 flex flex-col bg-white dark:bg-gray-900">
                <p className="text-sm font-bold text-gray-800 dark:text-gray-100 truncate mb-1">
                  {s.title || <em className="text-gray-400 font-normal">No title</em>}
                </p>
                {s.description && (
      ...
      >
        <p className="text-sm text-gray-700 dark:text-gray-300">
          Delete slide <strong>{deleteTarget?.order}</strong>? This cannot be undone.
        </p>
      </Modal>
    </div>
  );
